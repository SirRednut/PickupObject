...
using XInputDotNetPure; // Downloaded Package for use with GamePad
...
public class PickupObject : MonoBehaviour {
    ...
    private int additionFactor = 500;
    ...
    {
        if (objectInFront != null && objectSelected == null && timeSinceThrow >= 50 /*&& Vector3.Distance(objectInFront.transform.position, transform.position) < 5*/)
        {
    ...
    public void IncreasePower()
    {
        if (objectSelected != null)
        {
            ...
            // Calculate the vibration as a percentage between min and max force
            float vibration = (currForce - minForce) / (maxForce - minForce);

            // Max the vibration at 1
            if (vibration >= 1)
                vibration = 1;

            // Set the vibration in the rhs motor of the controller
            GamePad.SetVibration(0, 0, vibration);
        }
    }
    ...
    // Set up yhe Xinput to allow for controller vibration
    void XInputUpdate()
    {
